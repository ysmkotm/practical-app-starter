...
|BLG-EMP-007|EMP|社員番号の採番・形式・一意性ルール|社員番号の採番・形式・一意性の本検討・必要なら実装を行う。当面方針は設計書へ明記済み|Version 1.0 公開前（推奨：当面方針の明記）／本検討・実装は Version 1.x|保留|当面方針：[`EMP002_社員登録・編集.md`](../05_screen/EMP002_社員登録・編集.md) §5.2・§6、[`employee.md`](../04_db/table/employee.md) §2。再検討条件：Version 1.x。[`employee_code_logical_name.md`](../07_decisions/employee_code_logical_name.md)、[`progress.md`](progress.md) §8・§3|
